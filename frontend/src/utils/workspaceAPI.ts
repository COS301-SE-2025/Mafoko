...
export const bookmarkGlossaryAPI = {
  create: async (
    data: BookmarkedGlossaryCreate,
  ): Promise<BookmarkedGlossary> => {
  ...
  delete: async (domain: string): Promise<void> => {
    console.log('=== API DELETE GLOSSARY ===');
    console.log('Deleting glossary with domain:', domain);
    console.log('API endpoint:', API_ENDPOINTS.unbookmarkGlossary(domain));
    
    const response = await apiRequest(API_ENDPOINTS.unbookmarkGlossary(domain), {
      method: 'DELETE',
    });
...
export const workspaceGroupAPI = {
  ...
  getAll: async (): Promise<WorkspaceGroup[]> => {
    const response = await apiRequest(API_ENDPOINTS.getGroups, {
      method: 'GET',
    });
    const groups = await response.json() as WorkspaceGroup[];
    return groups;
  },

  update: async (
    groupId: string,
    data: WorkspaceGroupUpdate,
  ...
  addItem: async (
    groupId: string,
    data: WorkspaceGroupItemCreate,
  ): Promise<WorkspaceGroupItem> => {
  ...
  bulkAddTerms: async (
    ...
    const response = await apiRequest(API_ENDPOINTS.bulkAddToGroup(groupId), {
      method: 'POST',
...
export const workspaceOverviewAPI = {
  getOverview: async (): Promise<WorkspaceOverview> => {
    const response = await apiRequest(API_ENDPOINTS.workspaceOverview, {
      method: 'GET',
    });
