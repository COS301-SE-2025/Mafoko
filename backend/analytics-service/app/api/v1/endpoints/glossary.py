...
async def get_glossary_stats():
    ...
    return {
        "total_terms": len(df),
        "languages_count": len(language_columns),
        "categories_count": df["category"].nunique() if "category" in df.columns else 0,
        "languages": {lang: LANGUAGE_MAP[lang] for lang in language_columns},
    }


@router.get("/random")
async def get_random_term(count: int = 1):
    """Get a random term or set of terms."""
    df = await load_marito_data()

    # Get random samples
    random_terms = df.sample(min(count, len(df)))

    results = []
    for _, row in random_terms.iterrows():
        term_data = {
            "id": str(row.name),
            "term": row.get("eng_term", ""),
            "definition": row.get("eng_definition", ""),
