...
async def test_exact_prefix_match(db_session: AsyncSession):
    term = Term(id=uuid4(), term="Banana Farming", language="English", domain="Agriculture", definition="...")
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="Banana", fuzzy=False)
...
async def test_exact_prefix_match(db_session: AsyncSession):
    ...
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="Banana", fuzzy=True)
    assert any("Banana Farming" == t.term for t, _, _ in results)
...
async def test_exact_prefix_match(db_session: AsyncSession):
    ...
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="nana", fuzzy=True)
    assert any("Banana Farming" == t.term for t, _, _ in results)


@pytest.mark.asyncio
async def test_fuzzy_substring_match(db_session: AsyncSession):
    term = Term(id=uuid4(), term="Remote Sensing", language="English", domain="Science", definition="...")
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="mote", fuzzy=True)
...
async def test_fuzzy_substring_match(db_session: AsyncSession):
    ...
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="remote", fuzzy=False)
    assert any("Remote Sensing" == t.term for t, _, _ in results)


@pytest.mark.asyncio
async def test_no_prefix_match(db_session: AsyncSession):
    term = Term(id=uuid4(), term="Photosynthesis", language="English", domain="Biology", definition="...")
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="synthesis", fuzzy=False)
    assert len(results) == 0


@pytest.mark.asyncio
async def test_case_insensitive_search(db_session: AsyncSession):
    term = Term(id=uuid4(), term="Evapotranspiration", language="English", domain="Climate", definition="...")
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="EVAPO", fuzzy=False)
...
async def test_sort_by_name(db_session: AsyncSession):
    db_session.add_all([
        Term(id=uuid4(), term="Zebra", language="English", domain="Wildlife", definition="..."),
        Term(id=uuid4(), term="Antelope", language="English", domain="Wildlife", definition="...")
    ])
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="", sort_by="name")
    assert results[0][0].term == "Antelope"


@pytest.mark.asyncio
async def test_filter_by_domain(db_session: AsyncSession):
    term = Term(id=uuid4(), term="Organic Chemistry", language="English", domain="Chemistry", definition="...")
    db_session.add(term)
    await db_session.commit()
    results = await crud_search.search_terms_in_db(db=db_session, query="Organic", domain="Chemistry")
...
async def test_suggest_exact_prefix_only(db_session: AsyncSession):
    db_session.add_all([
        Term(id=uuid4(), term="Biology Basics", language="English", domain="Science", definition="..."),
        Term(id=uuid4(), term="Zoology", language="English", domain="Science", definition="...")
    ])
    await db_session.commit()
...
async def test_suggest_term_limit(db_session: AsyncSession):
    terms = [Term(id=uuid4(), term=f"Term{i}", language="English", domain="Test", definition="...") for i in range(15)]
