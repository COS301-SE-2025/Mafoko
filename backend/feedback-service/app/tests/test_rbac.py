...
class TestFeedbackRBAC:
    ...
    async def test_regular_user_cannot_access_admin_endpoints(
        self, client: httpx.AsyncClient, mock_regular_user
    ):
    ...
    async def test_user_can_only_see_own_feedback(
        ...
        feedback1 = await crud_feedback.create(
            ...
            obj_in=FeedbackCreate(
                type=FeedbackType.suggestion, message="User 1 feedback"
            ),
            user_id=user1_id,
        )
        feedback2 = await crud_feedback.create(
            db_session,
            obj_in=FeedbackCreate(
                type=FeedbackType.complaint, message="User 2 feedback"
            ),
            user_id=user2_id,
        )
    ...
    async def test_admin_can_access_any_feedback(
        self, client: httpx.AsyncClient, db_session: AsyncSession, mock_admin_user
    ):
        ...
        feedback = await crud_feedback.create(
            db_session,
            obj_in=FeedbackCreate(
                type=FeedbackType.suggestion, message="User feedback"
            ),
            user_id=user_id,
        )
    ...
    async def test_inactive_user_cannot_access_endpoints(
        ...
        def mock_inactive_user_dependency():
            raise HTTPException(
                status_code=400, detail="Inactive or locked user account."
            )

        app.dependency_overrides[deps.get_current_active_user] = (
            mock_inactive_user_dependency
        )
    ...
    async def test_permission_boundaries_clear(
        self, client: httpx.AsyncClient, db_session: AsyncSession
    ):
        ...
        user_feedback = await crud_feedback.create(
            db_session,
            obj_in=FeedbackCreate(
                type=FeedbackType.suggestion, message="User suggestion"
            ),
            user_id=user_id,
        )
