    ...
    *,
    ...
    for group in groups_result.scalars().all():
        ...
        term_count_result = await db.execute(
            select(func.count(GroupTerm.term_id)).where(GroupTerm.group_id == group.id)
        )
...
async def delete_group(
    *,
    ...
    await db.execute(delete(GroupTerm).where(GroupTerm.group_id == group_uuid))
    ...
    for term_id_str in terms_request.term_ids:
        try:
            ...
            skipped_terms.append(
                {"term_id": term_id_str, "reason": "Invalid UUID format"}
            )
            continue
        ...
        if not bookmark:
            skipped_terms.append(
                {"term_id": term_id_str, "reason": "Term not bookmarked by user"}
            )
            continue
        ...
        existing_result = await db.execute(
            select(GroupTerm).where(
                and_(GroupTerm.group_id == group_uuid, GroupTerm.term_id == term_uuid)
            )
        )
        ...
        if existing:
            skipped_terms.append(
                {"term_id": term_id_str, "reason": "Term already in group"}
            )
            continue

        # Add term to group
        group_term = GroupTerm(group_id=group_uuid, term_id=term_uuid)
        db.add(group_term)
...
async def remove_term_from_group(
    *,
    ...
    if not group_term:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Term not found in group"
        )
    ...
    for group_id_str in delete_request.group_ids:
        try:
            ...
            failed_groups.append(
                {"group_id": group_id_str, "reason": "Invalid UUID format"}
            )
            continue
        ...
        if not group:
            failed_groups.append(
                {"group_id": group_id_str, "reason": "Group not found"}
            )
            continue
