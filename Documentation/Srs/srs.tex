...
\subsection*{User Story #6: Term Validation System}

\textbf{ID:} US006 (Mavito Project) \\
\textbf{Title:} Crowdsourced Term Validation via Voting \\
\textbf{As a:} Mavito application user (e.g., linguist or contributor) \\
\textbf{I want:} to upvote or downvote individual terms in the lexicon \\
\textbf{So that:} I can signal the quality and accuracy of a term to the community and help surface the most reliable terminology.
...
\begin{enumerate}[label=\arabic*.]
    ...
    \begin{itemize}
        \item \textbf{Given} I am viewing a term on a term card,
        \item \textbf{Then} I see "thumbs-up" and "thumbs-down" buttons next to the term.
        \item \textbf{And} the current total upvote and downvote counts are displayed with the buttons.
    \end{itemize}

    \item \textbf{Casting a Vote:}
    \begin{itemize}
        \item \textbf{Given} I am logged in and have not yet voted on a specific term,
        \item \textbf{When} I click the "thumbs-up" button,
        \item \textbf{Then} my vote is recorded by the system, the upvote count increases by 1, and the button becomes highlighted.
    \end{itemize}

    \item \textbf{Changing a Vote:}
    \begin{itemize}
        \item \textbf{Given} I have already downvoted a term,
        \item \textbf{When} I click the "thumbs-up" button,
        \item \textbf{Then} my vote is changed from a downvote to an upvote, the downvote count decreases by 1, the upvote count increases by 1, and the button highlights are updated accordingly.
    \end{itemize}

    \item \textbf{Removing a Vote (Un-voting):}
    \begin{itemize}
        \item \textbf{Given} I have already upvoted a term,
        \item \textbf{When} I click the "thumbs-up" button again,
        \item \textbf{Then} my vote is removed entirely, the upvote count decreases by 1, and the button is no longer highlighted.
    \end{itemize}

    \item \textbf{Offline Voting:}
    \begin{itemize}
        \item \textbf{Given} I vote on a term while my device is offline,
        \item \textbf{Then} the UI should update optimistically to show my vote.
        \item \textbf{And} the vote action should be queued locally.
        \item \textbf{When} my internet connection is restored,
        \item \textbf{Then} the queued vote should be automatically sent to the server to be synced.
    \end{itemize}

    \item \textbf{Moderation Visibility (Future Enhancement):}
    \begin{itemize}
        \item \textbf{Given} I am a moderator or administrator,
        \item \textbf{Then} I can sort terms by "popularity" (upvotes - downvotes) to prioritize content review.
    \end{itemize}
\end{enumerate}
...
\textbf{Notes/Assumptions:}
\begin{itemize}
    \item Voting requires user authentication to prevent abuse and ensure each user can only vote once per term.
    \item Vote counts are public to all users to encourage transparency.
    \item The system is designed with a dedicated `vote-service` to handle all voting-related logic.
\end{itemize}
